...
from jrun.settings import find_jrun_dir, load_settings, save_settings
...
def submit(config_file, dry_run):
    ...
    if search:
        ...
        job_entries = []
        for i, j in enumerate(jobs, 1):
            click.echo(f"[{i}/{len(jobs)}] {j['name']}")
            if not _handle_name_conflict(j["name"], jrun_dir):
                click.echo("  Skipped.")
                continue
            click.echo(f"  Submitting...")
            job_id = _submit_one_job(j, exp_name, exp_id)
            if job_id:
                job_entries.append({
                    "job_id": job_id,
                    "name": j["name"],
        ...
        if not _handle_name_conflict(job["name"], jrun_dir):
            click.echo("Skipped.")
            return
        job_id = _submit_one_job(job, exp_name, exp_id)
        if job_id:
            record_single_job(jrun_dir, job_id, job["name"])
            click.echo(f"Submitted job {job_id}: {job['name']}")


def _handle_name_conflict(job_name: str, jrun_dir: Path) -> bool:
    """Check if a job with this name exists. Returns True if we should proceed, False to skip."""
    existing = find_job_by_name(jrun_dir, job_name)
    if not existing:
        return True

    old_id, old_info = existing
    old_status = old_info.get("status", "?")
    click.echo(f"  Job '{job_name}' already exists (id={old_id}, status={old_status})")
    choice = click.prompt(
        "  [o]verwrite / [s]kip?",
    ...
    # Overwrite: stop if running, then cancel
    if old_status in ("Submitted", "Running", "Queued", "Starting", "Scheduling"):
        click.echo(f"  Stopping old job {old_id}...")
        result = airsctl.job_stop(old_id)
        if result.returncode != 0:
            click.echo(f"  Stop skipped (job may have already finished)")
    click.echo(f"  Cancelling old job {old_id}...")
    result = airsctl.job_cancel(old_id)
    if result.returncode != 0:
        click.echo(f"  Cancel failed, proceeding anyway")
    remove_job(jrun_dir, old_id)
    return True


def _submit_one_job(job: dict, exp_name: str | None, exp_id: str | None) -> str | None:
    ...
    if rc and new_config.get("resource_config_list"):
        ...
        role = res.get("role_info_list", [{}])[0]
        detail = role.get("resource_request_detail", {})
...
def status(name_or_id):
    ...
    if not name_or_id:
        _show_all_jobs(tracker)
    elif name_or_id in tracker.get("searches", {}):
        _show_search_jobs(tracker, name_or_id, jrun_dir)
    else:
        _show_single_job(name_or_id)


def _show_all_jobs(tracker: dict):
    settings, jrun_dir = load_settings()
    ...
    for jid, info in jobs.items():
        ...
        if output:
            ...
            if status:
                info["status"] = status
                update_job_status(jrun_dir, jid, status)

    _print_job_table(jobs)


def _show_search_jobs(tracker: dict, search_name: str, jrun_dir: Path):
    ...
    for jid in search["job_ids"]:
        if jid in tracker["jobs"]:
            ...
            if output:
                status = _parse_job_status(output)
                if status:
                    tracker["jobs"][jid]["status"] = status
                    update_job_status(jrun_dir, jid, status)
...
def _show_single_job(job_id: str):
...
def _print_job_table(jobs: dict):
    if not jobs:
        return
    ...
    for jid, info in jobs.items():
        click.echo(
            f"{info['name']:<{name_width}} {info.get('status', '?'):<{status_width}} {info.get('submitted_at', ''):<20}"
        )
...
@cli.command()
@click.argument("name_or_id")
def cancel(name_or_id):
    """Cancel/archive a finished job by name or ID."""
    job_id = _resolve_job_id(name_or_id)
