...
# ==========================================================================
# Experiment (optional)
# The experiment to submit into: on the platform it is just a container for
# configs, so this block only carries its identity. On submit, jrun looks it
# up by name and creates it via the platform API if missing (using the first
# job's image/queue for the placeholder config). Requires credentials.
# On each submit, jrun replaces this experiment's config list with the selected
# current jobs; the local .jrun/jobs.json file retains job history and IDs.
# Do not share an experiment unless replacing its configs is intended.
# ==========================================================================
# experiment:
#   # (required) Experiment name.
#   name: my_experiment
#
#   # (optional) Explicit project IDs, skip API resolution when provided.
#   # proj_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
#   # projset_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
#
#   # (optional) Explicit cluster/zone, override queue resolution.
#   # cluster_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
#   # zone_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
#
#   # DEPRECATED: image/image_region/queue_name used to live here. They still
#   # work as fallbacks for jobs that don't set them, but prefer the job-level
#   # fields below (job.image / job.image_region / resource_config.queue_name).
...
# ==========================================================================
search:
  # (optional) Search name for tracking via `jrun job status <name>`.
  ...
  job_template:
    ...
    # (optional) Docker image jobs run with. Overrides the experiment template;
    # supports {param} substitution. queueId/cluster/zone are resolved via the
    # platform API from resource_config.queue_name (see `jrun queues`).
    image: harbor.platform.baai-inner.ac.cn/library/pytorch-sshd:23.08

    # (optional) Image repo type: PUBLIC (default) or PRIVATE.
...
job:
  ...
  # (optional) Docker image the job runs with. Overrides the experiment
  # template; queueId/cluster/zone are resolved via the platform API from
  # resource_config.queue_name (see `jrun queues`).
  image: harbor.platform.baai-inner.ac.cn/library/pytorch-sshd:23.08
